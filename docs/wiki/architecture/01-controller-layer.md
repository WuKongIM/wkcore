...
> 职责：维护集群拓扑、调度 Group 层的副本分布、编排修复/重平衡任务

## 1. 概述

Controller 是整个分布式系统的"大脑"。它运行一个独立的 Raft Quorum（通常 3 或 5 节点），不依赖 Group 层或 Channel 层的可用性。它的核心功能是：

1. **节点健康管理**：跟踪所有节点的心跳，维护 Alive / Suspect / Dead / Draining 四个状态。
2. **副本放置决策**：为每个 Group 计算期望副本分布（GroupAssignment）。
...
```
1. 修复（Repair）：优先级最高
   - 遍历所有 Group，检查是否有副本在 Dead/Draining 节点上
   - 找到第一个需要修复的 Group → 选新目标节点 → 生成 RepairTask

2. 重平衡（Rebalance）：修复完成后才执行
   - 计算各节点负载（承载的 Group 数量）
   - maxLoad - minLoad > RebalanceSkewThreshold（默认 2）时触发
   - 从最重节点选一个 Group 迁移到最轻节点
```
...
Controller StateMachine
  ...
  ▼
Node Agent (groupAgent)                           // agent.go:89-249
...
| 步骤            | 说明                                        |
| --------------- | ------------------------------------------- |
| AddLearner      | 将目标节点作为 Learner 加入 Raft Group     |
...
每个节点都运行一个 `groupAgent`，职责：

1. **周期上报**：每 200ms 通过 RPC 向 Controller Leader 发送心跳（`observeOnce()` → `HeartbeatOnce()`）
2. **同步分配**：拉取最新的 GroupAssignment，对比本地状态，Open/Close 相应的 ManagedGroup
...
| GroupAssignment  | `assignment/<groupID>`       | 期望副本集 + ConfigEpoch       |
| GroupRuntimeView | `runtime_view/<groupID>`     | 当前实际状态 + LeaderID        |
| ReconcileTask    | `task/<groupID>`             | 迁移进度 + 重试信息            |
...
| GroupCount            | uint32        | 必填         | MultiRaft Group 总数              |
| GroupReplicaN         | int           | 必填         | 每个 Group 的目标副本数           |

### 7.2 Planner 配置（groupcontroller/types.go:23-29）

| 参数                     | 类型           | 默认值 | 说明                                  |
| ------------------------ | -------------- | ------ | ------------------------------------- |
| GroupCount               | uint32         | -      | 管理的 Group 总数                     |
| ReplicaN                 | int            | -      | 期望副本数                            |
...
| 场景                      | 行为                                                          |
| ------------------------- | ------------------------------------------------------------- |
| Controller 不可用         | 现有 Group 继续运行，但不会有新的调度决策                     |
...
| Quorum 丢失               | Group 标记为 Degraded，不执行自动修复，需人工恢复            |
