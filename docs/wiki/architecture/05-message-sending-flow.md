...
- **读路径保持纯读取**：当前阶段只有发送链路的 refresh path 会触发 runtime-meta bootstrap；普通元数据读路径仍然只返回权威存储中的现状，不主动创建业务或运行时元数据。
...
| Channel 元数据过期或缺失 | `sendWithMetaRefreshRetry` 执行 `ErrNotFound -> bootstrap -> re-read -> apply -> retry` |
