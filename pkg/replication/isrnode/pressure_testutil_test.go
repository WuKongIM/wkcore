...
type pressureHarness struct {
	...
	queueHighWatermark           map[isr.NodeID]int
...
func newPressureHarness(tb testing.TB, cfg pressureConfig, mutate func(*Config)) *pressureHarness {
	...
	h := &pressureHarness{
		...
		clock:                        clock,
		throttle:                     throttle,
		queueHighWatermark:           make(map[isr.NodeID]int),
...
func (h *pressureHarness) initPeers() {
	h.peerIDs = make([]isr.NodeID, 0, h.cfg.peers)
	for i := 0; i < h.cfg.peers; i++ {
		h.peerIDs = append(h.peerIDs, isr.NodeID(i+2))
	}
}

func (h *pressureHarness) initGroups() {
	replicas := make([]isr.NodeID, 0, len(h.peerIDs)+1)
...
func (h *pressureHarness) peerFor(groupIndex, round int) isr.NodeID {
...
func (h *pressureHarness) deliverPeerFetchResponses(peer isr.NodeID, limit int) int {
...
func (r *pressureReplica) InstallSnapshot(_ context.Context, _ isr.Snapshot) error {
	return nil
}

func (r *pressureReplica) Append(_ context.Context, _ []isr.Record) (isr.CommitResult, error) {
	return isr.CommitResult{}, nil
}

func (r *pressureReplica) Fetch(_ context.Context, _ isr.FetchRequest) (isr.FetchResult, error) {
	return isr.FetchResult{}, nil
}

func (r *pressureReplica) ApplyFetch(_ context.Context, _ isr.ApplyFetchRequest) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.applyFetchCalls++
	return nil
...
func (t *pressureTransport) Send(peer isr.NodeID, env Envelope) error {
	return nil
}
...
type pressurePeerSessionManager struct {
	mu    sync.Mutex
	cache map[isr.NodeID]*pressurePeerSession
}

func newPressurePeerSessionManager() *pressurePeerSessionManager {
	return &pressurePeerSessionManager{
		cache: make(map[isr.NodeID]*pressurePeerSession),
	}
}

func (m *pressurePeerSessionManager) Session(peer isr.NodeID) PeerSession {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *pressurePeerSessionManager) session(peer isr.NodeID) *pressurePeerSession {
	m.mu.Lock()
