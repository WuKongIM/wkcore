...
func TestChannelStoreExposesStructuredQueryAPIs(t *testing.T) {
	...
	firstPayload := mustEncodeStoreMessage(t, channel.Message{MessageID: 31, ClientMsgNo: "same", FromUID: "u1", ChannelID: st.id.ID, ChannelType: st.id.Type, Payload: []byte("one")})
	secondPayload := mustEncodeStoreMessage(t, channel.Message{MessageID: 32, ClientMsgNo: "other", FromUID: "u2", ChannelID: st.id.ID, ChannelType: st.id.Type, Payload: []byte("two")})
	thirdPayload := mustEncodeStoreMessage(t, channel.Message{MessageID: 33, ClientMsgNo: "same", FromUID: "u3", ChannelID: st.id.ID, ChannelType: st.id.Type, Payload: []byte("three")})
