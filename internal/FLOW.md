...
| `access/node.Client` | `access/node/options.go:111` | 节点间 RPC 客户端：投递提交/推送/确认、Presence、会话事实查询 |
| `access/node.Adapter` | `access/node/options.go:64` | 节点间 RPC 服务端：注册到集群 RPCMux，处理入站 RPC |
...
| `Client` | access/node/options.go:111 | 节点间 RPC 客户端：投递/Presence/会话事实 |
| `Adapter` | access/node/options.go:64 | 节点间 RPC 服务端适配器 |
...
New(Config):
  → build(cfg):
    ...
    ⑫ 创建 Node Access: accessnode.New (注册节点间 RPC Handler)
...
message.App.Send(ctx, cmd):
  ...
  ⑨ sendDurable(ctx, cmd):
     ...
     b. sendWithMetaRefreshRetry: 带元数据刷新重试的 Append
        ...
        → 失败且 ErrStaleMeta → refresher.Refresh → 重试
...
- **sendWithMetaRefreshRetry**: 消息 Append 遇到 `ErrStaleMeta` 时会自动刷新 Channel 元数据并重试。这处理了 Channel Leader 迁移后本地缓存过期的场景。
